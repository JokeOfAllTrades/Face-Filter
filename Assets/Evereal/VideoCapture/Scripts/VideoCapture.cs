...
namespace Evereal.VideoCapture
{
  ...
  public class VideoCapture : VideoCaptureBase
  {
    ...
    public override void StartCapture()
    {
      ...
      {
        Debug.LogWarning("[VideoCapture::StartCapture] Previous capture not finish yet!");
        return;
      }
      //if (mode == ModeType.LIVE_STREAMING)
      //{
      ...
      //  {
      //    Debug.LogWarning(
      //       "[VideoCapture::StartCapture] Video live streaming require rtmp server address setup!"
      //    );
      //    return;
      //  }
      ///}
      ...
      {
        Debug.LogWarning(
            "[VideoCapture::StartCapture] Capture equirectangular video always require dedicated camera!"
        );
        isDedicated = true;
      }
      if (mode == ModeType.LOCAL)
      ...
      else if (format == FormatType.PANORAMA)
      {
        ...
        else
        {
          ...
          if (captureGUI)
          {
            ...
            faceTempRenderTexture.antiAliasing = antiAliasing;
      ...
      {
        Debug.LogWarning("[VideoCapture::StartCapture] Get native capture api failed!");
        return;
      }
      if (offlineRender)
      {
    ...
    private void OnRenderImage(RenderTexture source, RenderTexture destination)
    {
      ...
      if (status != VideoCaptureCtrl.StatusType.STARTED ||// Capture not started yet.
          (format != FormatType.PANORAMA && stereo == StereoType.NONE) ||
          // Whether is stereo video capture process (unsupported panorama).
          isDedicated)
        return;
    ...
    private void OnDestroy()
    {
      ...
      if (faceTempRenderTexture != null)
      {
        RenderTexture.Destroy(faceTempRenderTexture);
      }
    }
    /// <summary>
    ...
    private void FrameEncodeThreadFunction()
    {
      ...
      if (VideoCaptureCtrl.instance.debug)
      {
        Debug.Log("[VideoCapture::FrameEncodeThreadFunction] Encode process finish!");
      }
      // Update current status.
      status = VideoCaptureCtrl.StatusType.FINISH;
    }
