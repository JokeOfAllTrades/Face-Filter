    ...
    public class BarcodeDetectorWebCamTextureExample : MonoBehaviour
    {
        ...
        void Update()
        {
            if (webCamTextureToMatHelper.IsPlaying() && webCamTextureToMatHelper.DidUpdateThisFrame())
            {
                ...
                bool result_detection = detector.detectAndDecode(rgbaMat, decoded_info, decoded_type, corners);
        ...
        private void DrawBarcodeResults(Mat frame, Mat corners, List<string> decoded_info, List<int> decoded_type)
        {
            if (!corners.empty())
            {
                using (Mat corners_32S = new Mat())
                {
                    ...
                    for (int i = 0; i < corners_32S.rows(); i++)
                    {
                        ...
                        if (decoded_info.Count > i)
                        {
                            ...
                            {
                                Debug.Log("TYPE: " + BarcodeType(decoded_type[i]) + " INFO: " + decoded_info[i]);
                            }
                            else
                            {
        ...
        private void DrawBarcodeContour(Mat color_image, MatOfPoint corners, string decoded_info = "", int decoded_type = 0)
        {
            if (!corners.empty())
            {
                bool decodable = decoded_type != Barcode.NONE;
                ...
                Imgproc.putText(color_image, BarcodeType(decoded_type), new Point(p[0] + 5, p[1] - 5), Imgproc.FONT_HERSHEY_SIMPLEX, 0.7, new Scalar(255, 255, 255, 255), 2, Imgproc.LINE_AA, false);
        ...
        private string BarcodeType(int decoded_type)
        {
            string typeStr = "";
            switch (decoded_type)
            {
                ...
                case Barcode.UPC_E:
                    typeStr = "UPC_E";
                    break;
                case Barcode.UPC_EAN_EXTENSION:
                    typeStr = "UPC_EAN_EXTENSION";
                    break;
                case Barcode.NONE:
                default:
                    typeStr = "NONE";
